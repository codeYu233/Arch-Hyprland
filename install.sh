...
whiptail --title "codeyu233 Arch-Hyprland (2025) Install Script based on codeyu233's" \
    --msgbox "Welcome to codeyu233 Arch-Hyprland (2025) Install Script!!!\n\n\
...
echo "👌 ${SKY_BLUE}Continue with the installation...${RESET}" | tee -a "$LOG"
...
if ! pacman -Qs pciutils > /dev/null; then
    echo "${NOTE} - pciutils is not installed. It's necessary for detecting GPU. Installing..." | tee -a "$LOG"
...
while true; do
    ...
    if [[ "$dots_selected" == "OFF" ]]; then
        ...
        if ! whiptail --title "Hyprland Dot Files" --yesno \
        "You have not selected to install the pre-configured codeyu233's Hyprland dotfiles.\n\nKindly NOTE that if you proceed without Dots, Hyprland will start with default vanilla Hyprland configuration and I won't be able to give you support.\n\nWould you like to continue install without Hyprland Dots or return to choices/options?" \
...
for option in "${options[@]}"; do
    case "$option" in
        ...
        rog)
            ...
            ;;
        dots)
            echo "${INFO} Installing pre-configured ${SKY_BLUE}codeyu233's Hyprland dotfiles...${RESET}" | tee -a "$LOG"
...
if pacman -Q hyprland &> /dev/null || pacman -Q hyprland-git &> /dev/null; then
    ...
    printf "${SKY_BLUE}Thank you${RESET} 🫰 for using ${MAGENTA}codeyu233's Hyprland Dots${RESET}. ${YELLOW}Enjoy and Have a good day!${RESET}"
    printf "\n%.0s" {1..2}
